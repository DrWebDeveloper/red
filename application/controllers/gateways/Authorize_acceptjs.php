...
class Authorize_acceptjs extends App_Controller
{
     ...
     public function payment_new()
    {
       ...
       // $invoice = $this->invoices_model->get($this->input->get('invoiceid'));
        //load_client_language($invoice->clientid);
          $total_payment=$this->input->get('item_id');
          $decoded=base64_decode($total_payment);
        $data['invoice']  = "";
        $data['total']    = $decoded;
        $data['testMode'] = $this->authorize_acceptjs_gateway->getSetting('test_mode_enabled') == '1';
        echo $this->get_html2($data);
    }
     ...
     * @return string
     */
     protected function get_html2($data = [])
    {
        ob_start(); ?>
                    ...
                    <div class="panel_s">
                       <div class="panel-body">
                         <!--  <h4 class="no-margin">
                          ...
                          <h4 class="mbot20">
                             <?php 
                             echo "Payment_total: ".$data['total'].".00";
                            // echo _l('payment_total', app_format_money($data['total']); ?>
                          </h4>
