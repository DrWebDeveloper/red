                ...
                <tbody>
                    <?php foreach($attachments as $type => $attachment){
                        ...
                        <?php foreach($attachment as $_att){
							...
							$actual_link = substr("https://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]",0,44);
                            $au = "https://ikoproject.com/admin/clients/client/";
							if($au == $actual_link){
								$attachment_url = $url . $_att['file_name'];
							}else{
                                ...
                                <a href="<?php if($is_image){ echo isset($lightBoxUrl) ? $lightBoxUrl : $img_url; } else {echo $attachment_url; } ?>"<?php if($is_image){ ?> data-lightbox="customer-profile" <?php } ?> class="display-block mbot5">
                                    ...
                                    <?php } else { ?>
                                     <i class="<?php echo get_mime_class($_att['filetype']); ?>"></i> <?php 
                                        if(substr($_att['file_name'], -4) == ".txt"){
                                            echo "View Order File <a class='btn' href='".base_url().$_att['file_name']."' download><i class='fa fa-download' aria-hidden='true'></i> Download txt File</a>";
                                        }else{
                                        echo $_att['file_name'];
                                        }
                                         ?>
                        ...
                        <td style="text-transform: capitalize;text-align:center;"><?php 
                        if($_att['payment_status'] == "completed"){
                            echo $_att['payment_status'];
                        }else{
                            echo "-";
                        }
                         ?></td>
                        <td><?php echo $_att['payment_amount']; ?></td>
                        <td style="text-transform: capitalize;"><?php echo $_att['payment_source']; ?></td>
                        <td data-order="<?php echo $_att['dated']; ?>"><?php echo _dt($_att['dated']); ?></td>
